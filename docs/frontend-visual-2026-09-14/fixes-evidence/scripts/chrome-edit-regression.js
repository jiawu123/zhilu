async (page) => {
 ...
 await page.getByLabel('开始日期',{exact:true}).fill('2026-09-27');
 await page.getByLabel('截止日期',{exact:true}).fill('2026-09-30');
 await page.locator('.node-edit-form select').selectOption('ready');
 assert((await read()).plan.version===original.plan.version,'Chrome status write before save');
 await page.keyboard.press('Escape');
 await page.getByRole('dialog',{name:'放弃未保存的修改？'}).waitFor();
 await page.keyboard.press('Escape');
 await page.getByRole('dialog',{name:'放弃未保存的修改？'}).waitFor({state:'hidden'});
 assert(await page.getByLabel('开始日期',{exact:true}).inputValue()==='2026-09-27','cancel discarded date draft');
 ...
 await page.reload();
 await page.getByRole('button',{name:new RegExp('^\\d+\\.\\d+ '+title+'$')}).click();
 assert(await page.getByLabel('开始日期',{exact:true}).inputValue()==='2026-09-27','Chrome start date not persisted');
 assert(await page.getByLabel('截止日期',{exact:true}).inputValue()==='2026-09-30','Chrome deadline not persisted');
 assert(await page.locator('.node-edit-form select').inputValue()==='ready','Chrome status not persisted');

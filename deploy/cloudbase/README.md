...
- 云端仅准备了本地代码部署表单：`zhilu-staging`、端口 `8080`、持续运行 1 实例、COS 挂载到 `/mnt/zhilu`。尚未提交部署，不能当作已上线。
- 剩余：COS 连接凭证与私有权限、运行费用确认、OAuth App ID / App Key、已登记的回调地址、云端模型和知乎 CLI 凭证、真实长请求与登录验收。

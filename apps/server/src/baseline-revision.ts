...
import { prepareRoadmapperInput, type LiveResearchInput } from "@zhilu/agent-runtime";
...
export async function reviseBaseline(plan: PlanState, proposal: BaselineProposal, routeId: string, message: string,
  ...
  reportProgress("我正在结合你的调整意见和已有证据，修改计划草稿…");
  const output = await provider.generate(revisionInput);
  const next = await compileRoadmapperWithCorrection(plan, research, revisionInput, output, provider);

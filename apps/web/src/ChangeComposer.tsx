...
export function ChangeComposer({ context, busy, pending, error, onResetContext, onSubmit, onEditHours, children, expanded = false, maxLength = 4000 }: {
  children?: ReactNode; expanded?: boolean; maxLength?: number;
  ...
  return <section className={`change-composer ${expanded ? "is-expanded" : ""}`} aria-label="计划变更对话">
    {children}
    {error && <ErrorNotice message={error} className="composer-error" />}
    <form onSubmit={async event => { event.preventDefault(); if (!busy && description.trim() && description.length <= maxLength && await onSubmit(description.trim(), context)) setDescription(""); }}>
      ...
      <textarea ref={input} aria-label="变更说明" disabled={busy} maxLength={maxLength} rows={2} value={description} onChange={event => setDescription(event.target.value)} placeholder="请描述需要调整的事项及相关情况。" onKeyDown={event => {
      ...
      <div className="composer-actions"><button type="button" className="composer-hours" disabled={busy} onClick={onEditHours}><svg viewBox="0 0 20 20" fill="none" aria-hidden="true"><circle cx="10" cy="10" r="7"/><path d="M10 6v4l3 2"/></svg>时间约束</button>
        <span aria-live="polite">{busy ? "正在处理变更说明" : pending ? "变更预览待确认" : "计划修改经确认后生效"}</span>
        <button className="composer-send" type="submit" disabled={busy || !description.trim() || description.length > maxLength} aria-label="提交变更说明"><svg viewBox="0 0 20 20" fill="none" aria-hidden="true"><path d="M10 15V5m-5 5 5-5 5 5"/></svg></button></div>
    </form>
  </section>;
}

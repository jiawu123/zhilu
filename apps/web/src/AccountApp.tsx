...
export function AccountApp() {
  ...
  const refresh = () => { setError(""); void read<AuthStatus>("/api/auth/status").then(setAuth).catch(() => setError("无法连接本地服务，请检查后端后重试。")); };
  useEffect(refresh, []);
  ...
  if (!auth) return <main className="account-login"><h1>知路</h1><p>{error || "正在读取登录状态…"}</p>{error && <button onClick={refresh}>重试连接</button>}</main>;
  if (!mayEnter) return <main className="account-login"><p className="section-kicker">你的目标与成长记录</p><h1>登录知乎，继续你的计划</h1><p>访谈回答和计划历史会保存在你的账号下。</p>
  ...
  return <><App key={auth.accountKey} interviewStorageKey={storageKey} onOpenHistory={() => void loadHistory()} />
    {open && <WorkspaceDialog title="我的历史记录" onClose={() => setOpen(false)}>
      ...
      {planHistory ? <><button onClick={() => setPlanHistory(null)}>← 返回列表</button><h3>计划调整对话</h3>
        {planHistory.length === 0 && <p>还没有已保存的计划调整对话。</p>}
        {planHistory.map(item => <article key={item.id}><strong>{new Date(item.createdAt).toLocaleString()}</strong>{item.conversation.length === 0 ? <p>首次计划草稿</p> : item.conversation.map((message, i) => <p key={i}>{message.role === "user" ? "我" : "知路"}：{message.content}</p>)}</article>)}
      ...
      </> : <><h3>目标访谈</h3>{history?.interviews.length === 0 && <p>还没有访谈记录。</p>}

...
const labels = { single: "单选", multiple: "可多选", text: "填写回答", toggle: "选择一个程度" };
...
export function InterviewQuestionCard({ question, number, answer, disabled, onChange }: {
  ...
  return <fieldset className={skipped ? "question-is-skipped" : ""} disabled={disabled}>
    ...
    {type === "text" ? <label className="text-answer">你的回答
      <textarea maxLength={1000} disabled={skipped} value={answer?.text ?? ""} onChange={event => onChange({ questionId: question.id, text: event.target.value })} placeholder="简单描述即可，也可以跳过" />
    ...
    {needsOtherText(question, answer) && <label className="other-answer">补充“其他”的具体情况
      <textarea maxLength={1000} value={answer?.text ?? ""} onChange={event => onChange({ ...answer!, text: event.target.value })} placeholder="请填写你的实际情况" />
    </label>}

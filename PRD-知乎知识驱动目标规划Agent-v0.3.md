...
| 输入来源 | 用户需求、`SOUL.md`、Plan Debugger v0.3、OpenClaw 个人助手 PRD；官方赛事指南截图与指定 Skill 包（见附录 C） |
| 本次修订 | 补充参赛公网交付、OAuth 与凭据边界、接口及配额口径、Controller 闭环、M3/M4 真联调及刘看山轻量交互 |
| 状态 | 待团队评审与实施验收；已补充官方提报要求，实际账号配额、素材使用说明及线上联调仍待核验 |

本次新增条目是需求与验收条件，不表示代码、认证、部署或作品提交已经完成。官方要求、产品设计决策和待验证事项分别标注；资料版本与核验边界见附录 C。
...
MVP 的开发与个人使用形态是运行在用户本机的 Web 应用；参赛交付另须提供公网可操作 Demo，不能以 localhost 地址代替作品链接。二者复用同一套知识链路、Plan Engine 和 Plan Bundle，不另建一套演示逻辑：

```text
用户
  ↓
浏览器前端：访谈页 / 计划确认与对话页 / Roadmap / 依据检查器 / Impact Review / History
  ↓ localhost API / SSE
本地服务
  ├── 知识与 Agent 层
  │     ├── 自适应访谈
...
浏览器打开 `http://localhost:<port>`。个人本地使用不需要购买公网服务器；参赛 Demo 则必须在提交前完成公网部署与独立浏览器验收。具体托管方式待确定，不要求购买云服务器，但必须提供稳定的公网 HTTPS 入口、受控后端和隔离的体验数据，不能直接公开现有本机项目目录与未鉴权 API（见 §9.6、§15）。
...
Roadmap 是面积最大的连续区域。上图展示面板展开状态；进入工作区时 Sidebar 与依据检查器默认收起，提供清晰的 toggle 开关。Sidebar 只承载导航、Sources、History、设置和待确认事项；依据检查器在用户选中节点时按需展开。默认只显示目标、图、少量必要入口，不同时铺开设置、导出、历史和检索参数。P0 验收时，Roadmap 主工作区在桌面宽度下应占可用内容宽度的 70% 以上，用户执行计划时不需要切回 Chat 或离开主视图。
...
P0 包含独立的访谈页、计划确认页与 Roadmap 执行页。候选路线、计划草稿和确认前对话位于计划确认页；Roadmap 在用户确认后展示正式计划的日期、依赖和进度。底层 Plan Bundle 不绑定具体视图。P1 可基于同一份 Plan Data 增加：Flow（依赖与因果）、Routes（候选路线与当前选择）、Timeline/Gantt（日期、Deadline 与时间冲突）。任何视图中的修改都必须经过同一套 Plan Engine 更新 Plan Bundle，不能分别维护四套数据。

#### 3.2.1 刘看山与图上反馈（产品设计决策）

刘看山是知乎官方吉祥物 IP，不是新的 Agent 或功能模块。优先检查并使用官方提供的「看山三视图.zip」「刘看山动态.zip」，核对包内素材和使用说明后接入；不能把提供赛事素材推定为无限制的赛外使用许可，也不能暗示本产品由知乎官方出品。

| 场景 | 最小交互 | 边界 |
| --- | --- | --- |
| 首次进入 | 在路线起点旁展示小型角色与一句目标引导 | 不增加全屏引导或强制点击步骤 |
| 检索与生成 | 在画布边缘展示合适的官方动态素材及当前处理阶段 | 阶段来自真实运行状态；不伪造检索数、百分比或成功结果 |
| 完成节点 | 在刚完成的节点附近短暂庆祝，随后恢复静止 | 不持续循环、不改变节点位置 |
| 计划受阻 | 在受影响区域旁提示，用户点击后展开原因与 Diff | 不自动应用修改；不遮挡节点、依赖线或拖动区域 |

图的当前路线保持清晰，候选路线弱化，只有当前选中或受影响的节点展开详细信息。角色装饰不拦截画布平移、缩放或节点拖动；可交互提示提供键盘入口。尊重 `prefers-reduced-motion`，使用静态替代；素材缺失或加载失败时保留文字反馈和完整操作能力。P0 只做轻量引导与反馈，不增加养成、积分、角色聊天或新的导航层。

### 3.3 辅助界面

1. **访谈页（独立 page）**：输入目标，回答模型按轮生成的背景题，并编辑确认目标与背景摘要。
2. **计划确认页（独立 page）**：展示知乎证据支持的候选计划、具体任务与验收标准；旁边提供对话框调整草稿。用户确认后才进入 Roadmap 并生成正式 Plan Bundle。草稿不嵌入 Roadmap，也不作为 Roadmap 弹窗。
3. **依据检查器（Source Inspector）**：显示节点为何存在、来源类型、原文链接、适用条件、验证状态、风险标签、冲突观点和最终采用原因。节点没有知乎来源时，仍需标明它来自用户事实、规则或 AI 推断。
4. **Impact Review**：集中查看突发事件造成的 Before → After Diff。
5. **History**：查看 Roadmap 每个版本的修改人与修改原因。

### 3.4 后续形态

- **P1 分享页**：将脱敏后的 Plan Bundle 发布为只读网页或导出静态包；这是用户计划分享功能，不是 P0 必交的公网作品体验链接。
...
→ 模型根据目标生成首批 1–5 道背景题
→ 用户答完本轮，提交题目及已选答案
→ 后端把目标与累计问答交给模型，生成下一批 1–5 题或结束（累计最多 30 题）
→ 用户编辑并确认“目标与背景摘要”
→ 进入独立计划页：检索知乎，生成证据卡与候选计划
→ 用户检查路线、任务、时间、产出、验收与依据
→ 在旁边的对话框提出调整，模型利用现有证据更新草稿，可多轮讨论
→ 用户确认 Baseline
...
- 首批问题根据用户目标生成；不使用固定题库或写死的目标类别分支。
- 每轮允许 1–5 道题。本轮所有题回答或跳过后才调用一次模型 API，输入目标、历轮题目及对应答案和跳过标记；模型根据新信息决定下一批问题、题型或结束。
- 全程最多 30 道题，信息足够可提前结束；不要求凑满 5 的倍数。例如共 22 题时按 5、5、5、5、2 展示，最后两题回答后生成摘要。
- 后端保存会话并校验当前题目和选项、完整提交、重复提交及剩余题数；前端不得通过覆盖历史绕过上限。
- 题型由模型决定：单选、多选、填空，或互斥的程度 toggle 按钮。选择题和程度题提供 2–6 个清晰选项，允许“不确定／不适用”；选中“其他”后显示必填补充文本框。全程填空题最多 3 道，跳过也计入此上限；“其他”的补充不算独立填空题。
- 每题下方提供“跳过问题”和恢复作答入口；跳过记录为未知，不推断成否定答案。文本回答及“其他”补充各最多 1,000 字符。模型不得把未回答内容变成用户事实；缺少的期限或投入只能作为明确标注、待用户编辑确认的建议。
- 最后一轮答完后调用模型输出可编辑摘要；它不是额外背景题，且不自动批准。用户确认摘要后才开始知乎研究。
- 请求等待期间显示旋转圆、已等待秒数；失败、断连或超时停止动画并展示具体错误，保留本轮选择以便重试。进度条显示已回答或跳过题数，以 30 题为上限并注明可提前结束，结束时按实际题数显示完成；不冒充模型执行百分比。模型失败不退回硬编码题库。刷新可恢复服务端已保存的访谈批次。

### 5.1.1 问答与历史持久化

- 用户每题的选择、填空、“其他”补充和跳过先作为当前轮草稿自动保存，界面明确显示保存中、已保存或失败；停止输入 500 ms 后发送，未保存离页提示。
- 提交整批时先落盘答案，再请求模型。生成失败保留答案与失败记录，重试只生成下一批或摘要，不重复提交已确认答案。
- 保留所有题目和选项、批次提交时间、生成成功/失败事件、可编辑摘要及关联项目；已提交答案不得被后续模型覆写。
- “历史记录”列出当前账号的访谈和计划，可以查看逐题问答、继续访谈、打开关联计划及版本历史。刷新、重新登录后从服务端恢复，不依赖浏览器缓存保存正文。
- 账号数据保存在持久后端目录并隔离；旧本地未归属数据保留在本地，不自动归到首次登录账号。正式计划既有 Commit History 继续保留；确认前的计划调整对话独立存档，不随草稿替换或确认而删除。
...
→ 多样性约束
→ 各问题返回 EvidencePack
→ Controller 跨问题汇总，按全局预算选择 6–8 张 Evidence Card
→ 检查最终证据覆盖；有关键缺口时最多补一轮 1–3 个 Query
→ 重新汇总与检查；仍不足则明确缺口，不伪造证据或路线
→ Plan Compiler
```
...
Workflow Controller 不负责撰写 Research Question。它根据状态机判断是否需要研究，检查 Query Planner 输出的数量、必填字段和隐私边界，然后为每个 Research Question 分配 ID，补充相关用户条件、时效要求和 Evidence 数量上限，组装为 `ResearchRequest`。收到各问题结果后，它还负责跨问题证据汇总、覆盖检查及有预算的补检索调度（见 §6.12）。“生成问题内容”属于 Query Planner；“组装、校验和调度请求”属于 Controller。
...
- 高度相似的观点可关联展示各自来源；同源重复内容不计为多个独立来源，不能仅因重复出现就称为“共识”；P0 不为合并展示而丢失原始 Evidence ID 与来源；
...
```json
{
  ...
  "summary": "作者强调可观测性、评测、状态和错误恢复，而不只是 Demo 界面。",
  "sourceType": "zhihu",
...
依据附录 C 的赛事截图和指定 Skill 包，按需使用接口，不以“接入全部接口”为目标：

| 能力 | 产品用途 | 范围与限制 |
| --- | --- | --- |
| 知乎搜索 | P0 的主要外部证据来源 | 保留作者、链接、时间与摘要；`ContentText` 是摘要，不宣称读过回答全文 |
| 全网搜索 | 后续按需补充官网等外部依据 | 非 P0 必接；与知乎来源分开标注，不改变 §6.4 的事实验证边界 |
| 直答 Agent | 可选的综合解释能力 | 不默认替换搜索与 Controller；官方文档未保证本项目要求的结构化 EvidencePack 和引用关系 |
| 知乎热榜 | 可选的热点背景 | 非 P0 必接；热度不代表事实可靠或适合当前用户 |
| 知乎故事／知识 | 活动提供的指定内容集合 | 非全站任意主题知识库；包内文档列明的活动接口无需鉴权，不扩展为其他接口的规则 |
| 用户关注、收藏等 | 后续经用户明确授权的个性化输入 | 非 P0 必接；登录本身不代表同意读取全部个人资料 |

**额度不能写死为已获得的账号能力。** 赛事截图标注知乎搜索、全网搜索各 5,000 次/天，热榜、直答各 100 次/天；指定 Skill 的通用说明标注默认每租户、每能力组 100 次/天，低额度账号可能为 10 次/天。两种材料口径不同，赛事额度是否已分配给当前凭据仍待确认，以实际 `quota` 的 `TotalQuota`、`TotalUsed`、`RemainingQuota` 为准。只在配置验收、额度询问或失败排查时查询，不为每次请求额外轮询。

应用层必须缓存和去重，区分缓存命中、真实检索与模型调用。配额或频率受限时停止自动重复调用，展示受影响阶段和可用的已存结果；不能把鉴权失败或配额失败报成“未找到证据”。缓存回放必须明确标注，不能作为本次实时检索成功。禁止批量、高频、无意义地调用发布接口；本产品 P0 不自动向知乎发布内容。

### 6.12 Controller 的跨问题闭环（P0 待实施与验收）

Controller 属于 Core 编排层；Research Subagent 内部的 Query Planner 仍负责问题和 Query 内容。Controller 通过既有研究边界调用，不绕过 `ResearchRequest → EvidencePack` 直接拼接研究模块内部步骤。

1. **汇总**：合并各问题的 EvidencePack，按来源内容 ID、规范化 URL 和内容重复度去重；不同 ID 的同一内容不能重复占位。按 §6.6 选取全局证据，保留相反观点的适用条件。保留或明确映射 Evidence ID，检查 Route Candidate 的引用；失去依据的候选不能继续作为有证据路线传给 M3。
2. **覆盖判断**：针对最终选中的证据，检查关键问题、用户条件、路线依据及至少一项限制或反例。返回已覆盖项、缺口、对应 Evidence ID 和缺口影响的决定，不能把各问题的“充分”标记直接相加，也不能以“已有 8 张卡”代替语义判断。模型判断只是建议，不自动升级为已验证事实。
3. **补检索调度**：把缺口、已执行 Query 和剩余预算交给 Query Planner 的补检索能力；它只提出 Query，不自行放宽预算。Controller 校验隐私、相关性与去重后执行研究，再重新汇总与判断。

产品默认预算：每次首次研究首轮共 2–3 个 Query，最多补一轮 1–3 个 Query，跨所有 ResearchRequest 的总上限为 6 个，而不是每个问题各 6 个。后续局部研究也只允许一次补充轮次并受同一总上限约束。已向外部发出的失败请求计入尝试次数；缓存命中单独记账。每轮设总超时，并记录各阶段耗时、真实调用数、缓存命中和停止原因，不能无限补搜或自动重试配额错误。

证据不足时显示缺口并要求澄清或停止正式规划；可以保留明确标注的研究草案，但不得伪造第二条路线、补造来源或自动写入 Baseline。验收至少覆盖：跨问题重复、证据充分时零补搜、缺口触发一轮定向补搜、预算或配额耗尽后停止、仍不足时不写正式计划。
...
归档节点或 Route 时，只在 `plan.json` 中把状态改为 `Archived`；归档整个项目时，在 `manifest.json` 中把项目状态改为 `archived`。归档不删除 Bundle，也不改变其存储位置；本地模式不会因此上传数据，托管 Demo 仍使用该会话或用户的隔离存储。UI 默认隐藏并允许恢复。
...
- [ ] 首轮按目标生成 1–5 道背景题；
- [ ] 每轮提交后把累计问答交给模型，按需生成下一轮；全程不超过 30 题；
- [ ] 支持 22 题按 5、5、5、5、2 收尾，且模型失败时保留当前批次可重试；
- [ ] 能根据答案跳过无关问题；
- [ ] 能识别回答中的冲突并追问；
- [ ] 用户可以修改背景摘要；
- [ ] 未明确成功标准时不直接生成正式计划。
...
- [ ] 没有来源支持的判断标为 AI 推断。
- [ ] Controller 完成跨问题汇总、全局覆盖判断与最多一轮补检索，引用不因去重失效；
- [ ] 证据充分时不补搜；不足、超时、配额耗尽时显示真实状态且不自动写入正式计划；
- [ ] 冷启动与缓存回放分别记录调用数和耗时；脚本退出成功不能代替证据质量验收。
...
- [ ] 用户可以编辑并确认 Baseline；
- [ ] 可导出完整 Bundle。
- [ ] 使用一次真实 M2 产生的最小必要证据快照独立调用真实 M3 模型；回放期间不再次检索知乎，快照按用户／项目隔离并排除凭据；
- [ ] 模型输出通过字段、日期、工时、依赖和引用校验后才成为待确认提案；失败或未确认时正式版本不变。

独立证据回放入口属于待补开发，不能把当前连跑 M2＋M3 的页面入口描述为已经具备 M3 独立验收能力。
...
- [ ] 用户手工修改不会被静默覆盖；
- [ ] 每次正式调整有原因和历史记录。
- [ ] 默认两侧面板收起；刘看山反馈不阻挡平移、缩放、拖动或 Diff 审阅，减弱动态时可完整操作；
- [ ] 在已确认计划上把每周投入从 8 小时改为 4 小时，独立调用真实 M4 模型重排，知乎调用数为 0；
- [ ] 重排保留已完成任务与受保护的手工修改，检查时间预算和依赖；超出原目标期限时告警，不静默延长 Goal；
- [ ] Diff 确认前正式版本不变，应用后新增一个 Commit；刷新后 Roadmap 与 History 一致。

首个真实联调样例限于每周时间变化的局部排期，不据此宣称所有突发事件都支持模型重规划；其他事件按各自能力与验收记录标明边界。
...
M5 的 P1 分享不影响参赛交付：P0 仍须提交整个产品的公网可操作体验链接，见 §15.3。
...
| `POST /interviews` | 按目标创建会话并生成首批背景题 |
| `GET /interviews/:id` | 恢复已保存的访谈批次与问答 |
| `POST /interviews/:id/answers` | 提交本轮答案及跳过标记，生成下一批或背景摘要 |
| `POST /projects/:id/baseline/revise` | 根据对话调整候选草稿，复用证据，不写正式计划 |
...
后端负责调用知乎接口和模型，Access Secret 不进入浏览器页面。本地模式使用系统 Keychain 等安全存储；托管 Demo 使用部署平台 Secret 或受控服务端存储，不要求服务器具备桌面 Keychain。
...
5. Controller 跨问题汇总 EvidencePack、检查全局覆盖，并在预算内完成必要补检索；将选中的证据、引用有效的 Route Candidate、剩余缺口、Goal Contract 和必要用户条件交给 Roadmapper，关键证据不足则停止正式规划；
...
例如原计划依赖的学习资料失效，现有 Evidence 又没有替代路线。Controller 判断 `shouldResearch=true`，将“原资料失效且缺少替代路线”这一知识缺口摘要交给 Query Planner；Query Planner 生成“在当前目标和用户条件下有哪些可替代路线”等 Research Question。Controller 组装 `ResearchRequest` 并启动新的 Research Subagent Run。Controller 按 §6.12 汇总受影响问题的新旧证据、检查覆盖并在同一预算内完成必要补搜；关键依据仍不足时保留原计划并显示缺口。通过检查的证据与受影响 Plan 子图一起交给 Roadmapper，未受影响节点不进入 Context，也不重新生成整个计划。

### 9.5 UI CLI 与 MCP 的关系

UI、CLI 和 MCP 是 Plan Engine 的并列入口，不是依次连接。P0 Web UI 在本地模式通过 localhost API 调用 Engine，托管 Demo 通过 §9.6 的受控 HTTPS 后端调用同一 Engine。具有 Shell 权限的 Coding Agent 可以在 P1 使用 CLI；支持 MCP 的 Agent 可以通过 MCP Adapter 使用同一组受限能力，不需要同时接入 CLI 和 MCP。
...
### 9.6 参赛 Demo 的身份与部署边界

**官方要求与产品选择分开：** 公网可操作 Demo 是必交项；截图推荐知乎账号登录，登录数是最佳人气奖评定条件之一，但没有将 OAuth 写成所有作品的强制门槛。产品已明确选择接入知乎登录，并按账号保存访谈与计划历史；本地无 OAuth 配置仍可开发，真实授权未通过前不得宣称已完成登录联调。

| 用途 | 所需凭据 | 边界 |
| --- | --- | --- |
| 后端检索知乎 | Access Secret | CLI 使用此凭据所属账号，不等于应用用户登录 |
| 知乎登录与授权用户基础信息 | 赛事 App ID、App Key，登录后取得 OAuth Token | App ID 是应用标识；App Key 与 Access Secret 不同，只留在后端 |
| 读取已授权用户的关注、收藏等列表 | Access Secret ＋该用户 OAuth Token | 非 P0 必接；不得把凭据持有者的数据当作当前体验者的数据 |
| 模型规划与证据处理 | 模型服务 API Key | 与以上知乎凭据独立，不能相互替代 |

赛事 App ID／App Key 按赛事项目页面领取，开发期是否已有可领取入口需实际核验。接入以附录 C 的指定 beta 包中 `hackathon-oauth.md` 为依据：

- 回调地址与赛事登记值完全一致；使用公网 HTTPS 完成真实授权，最终授权由用户本人确认。
- 后端生成随机、短时、绑定浏览器会话的 `state`；回调中先校验并一次性消费 `state`，再处理 `authorization_code` 和 Token 交换。缺失、不匹配、过期、跨会话或重复回调必须拒绝。
- OAuth Token 留在服务端；浏览器仅持有应用会话标识，使用 `HttpOnly`、`Secure` Cookie。写操作校验会话与请求来源，不能把 CORS 当作身份认证。
- 退出、会话或 Token 失效后停止授权数据访问；不自动切换到开发者账号。多实例部署使用共享会话存储，不能依赖单进程内存维持跨实例登录。

**无论是否接 OAuth，公网发布都必须满足：**

- 项目读取、修改、History、导出与真实研究入口均校验用户或访客会话的项目归属，不能仅凭 URL 中的 `projectId` 访问任意项目。
- 不同体验者使用独立项目与私有证据缓存；演示模板只读，操作时创建各自副本。不公开开发者本机已有项目或私人材料。
- 真实调用设置会话／用户级与应用总预算、并发上限和服务端超时；缓存命中单独计数。超限时拒绝新调用，不自动重试；接口失败可查看已有结果但必须标明状态。
- 部署前配置允许的站点来源、存储位置与实际保存期限；在用户输入前说明数据将由托管后端处理，并提供删除方式，按公布期限回收体验数据。
- App Key、Access Secret、模型 Key、OAuth Token 不进入 Git、`VITE_*`、前端响应、日志、截图或视频。密钥只读自服务端安全配置，不能由请求参数选择其他用户的凭据。
...
21. 默认图优先、面板可 toggle，接入经核对的刘看山轻量反馈及静态降级；
22. 分别完成 M3/M4 真实模型验收与公网完整 Demo 验收，提交产品说明计划书、项目资料和可运行链接（§15）。

知乎 OAuth 推荐纳入参赛发布候选；是否接入须在提交前明确记录，接入时必须通过 §9.6 的真实授权验收，不能以 Mock 登录代替。它不改变公网 Demo 必交和用户／会话隔离必做的要求。
...
以上为性能目标，不是已测结果。真实联调按 Planner、搜索、证据处理、M3、M4 分别记录耗时，区分冷调用与缓存回放；单次成功或失败耗时不能当作 P95。等待时展示真实阶段及错误，刘看山动效不能代替进度或隐瞒超时。

### 12.2 隐私

- 本地模式的用户文件和 Plan Bundle 默认保存在本机；托管参赛 Demo 按 §9.6 使用隔离存储，并明确告知保存位置、期限及删除方式；
- 搜索 query 不带姓名、联系方式、公司机密和完整简历；
- 调用模型前显示会发送的数据类型；
- 分享前提供脱敏预览；
- 用户可以删除自己在当前模式下的项目与 Evidence Cache，不影响其他用户；
...
- 知乎凭据和模型 Key 只存后端安全配置，不进入前端 Bundle；本地与托管方式见 §9.6；
...
| R2 | 30 问导致用户流失 | 高 | 每轮 1–5 道选择题，信息足够即停止，全程最多 30 题 |
...
| R10 | 产品同时做 RAG、Planner、MCP、App 导致失控 | 高 | P0 聚焦知识到计划闭环与最小安全公网交付；CLI、MCP、多视图、只读计划分享和桌面封装后置 |
| R11 | 公网体验泄露项目或耗尽调用预算 | 高 | 会话与项目归属校验、隔离存储、服务端密钥、并发与总预算限制；上线前用两个独立会话测试 |
| R12 | 本地可运行但错过提报或线上不可用 | 高 | 提前确认托管、作品链接和材料；按 §15.3 在截止前发布并从外部网络验收 |
...
演示讲解控制在 3–5 分钟，真实调用等待时间单独记录，不以讲解时长宣称端到端延迟。先从公网作品链接进入；若已接入知乎登录，由体验者本人完成授权。使用缓存时明确显示“缓存回放”和证据获取时间，不能冒充本次实时检索：
...
11. 刷新页面，核对 Roadmap 与 History 的当前版本，再导出 Plan Bundle；
12. 如 P1 计划分享功能完成，可额外打开脱敏只读分享页；不能用该页面替代公网产品体验。
...
以下是后续工作顺序与验收门槛，不是当前完成状态。公网部署配置、OAuth 和提交材料可与核心联调并行准备：

| 阶段 | 验收内容 | 必须保留的结果 |
| --- | --- | --- |
| 本机环境与真实 M2 | 核对指定 CLI／Skill 版本、Access Secret、模型配置和实际额度；先离线回归，再受限真实检索 | 真实来源、请求状态、分阶段耗时、冷调用／缓存计数；不含密钥 |
| Controller 三步 | 跨问题去重、覆盖判断、定向补搜、预算与失败停止 | §6.12 的正反例测试；缺口和停止原因可检查 |
| 独立 M3 | 保存合规且最小化的真实证据快照，补独立回放入口，再调用真实模型生成路线 | 未重新检索知乎；工时、日期、依赖、交付物、验收标准和引用通过校验；确认前不写正式版本 |
| 独立 M4 | 从已确认计划触发每周时间减半，真实模型生成局部排期 Diff | 知乎调用为 0；保护手工修改和完成状态；超期显式告警；批准前后版本一致性可核对 |
| 公网完整 Demo | 从新会话完成目标确认、研究、证据、路线选择、图上操作、事件、Diff、确认、刷新和导出 | 一次完整运行记录、来源与版本核对；两会话隔离、失败降级与限额检查；已接 OAuth 时附真实登录／退出验证 |

不能把单元测试、Mock、M2 脚本退出成功、缓存回放或单个模型调用成功，等同于完整线上验收。任何阶段失败都保留真实状态；再次运行 M3 优先复用合法证据快照，避免无必要地重跑 M2。

### 15.3 官方提交要求与发布检查

**官方要求（依据附录 C）：** 作品提交窗口为 2026 年 9 月 13 日 10:00 至 9 月 15 日 10:00；截止后不接受项目文件、视频、计划书等补交。由队长在赛事页面创建、维护并最终发布项目；如页面有更新，以提交时官方显示为准。

- [ ] 项目名称、赛道、介绍、icon 和封面按提交页面要求填写；赛道选项及可选数量以实际页面为准；
- [ ] 必交公网可运行 Demo 链接，评委与社区用户可实际操作，而非 localhost、仓库链接或仅截图；
- [ ] 必交产品说明计划书：交代目标用户、核心问题、完整体验、技术方案、使用的知乎能力、证据如何影响计划，以及对知乎社区的实际价值；
- [ ] 产品说明注明当前能力、限制与真实／缓存演示边界，不把 PRD 或测试通过当成已上线证明；
- [ ] 按需提交代码仓库与演示视频，并检查评委可访问；二者不能替代必交材料；
- [ ] 明确是否接入知乎 OAuth；若接入，登记准确回调并完成真实授权、退出和异常状态测试；若未接入，不显示假的登录成功；
- [ ] 用外部网络或独立设备验证作品 HTTPS 地址和完整流程，两个会话无法越权读写、导出对方项目；
- [ ] 知乎／模型配额不足、超时、空证据、素材加载失败均有可理解的提示；无泄露凭据、私人材料或共享可写演示项目；
- [ ] 最终在截止前发布，并重新查看赛事项目页面确认作品与链接已生效；保存草稿不等于提交完成。

---

## 16. 开放问题

1. 指定 beta Skill 与部署环境的 CLI 是否兼容，当前凭据是否已获得赛事搜索额度？官方基线见附录 C，实际权限以调用结果为准。
2. 实际 API 响应的作者、时间、相关性与摘要字段是否齐全，可否满足证据筛选与引用需求？
3. 是否允许本地缓存回答片段，缓存期限和展示长度限制是什么？
4. 用户是否愿意在看到计划前完成首批 1–5 个选择题？
5. 黑客松是否要补充第二个明显不同的目标案例，以证明框架不是 Agent 工程师专用？
6. 是否需要把用户简历纳入 P0，还是只支持结构化自述？
7. 时间轴以 8 周、12 周还是按用户截止日期动态生成？
8. 公网 Demo 使用何种托管、会话存储、数据保存期限和调用预算？P0 公网交付已确定，P1 用户计划分享不作为前置条件。
...
12. 赛事 App ID／App Key 是否已可领取，正式回调地址是什么？OAuth 推荐接入，但不能把待领取凭据写成已打通。
13. 刘看山官方素材包含哪些可用动作、格式和使用限制？检查后选择轻量素材，不预设包内已有特定动画。
...
- [ ] 用户可在本地 Web 创建项目；
- [ ] 公网 Demo 可由独立会话实际操作，具备项目归属校验、体验数据隔离和真实调用保护；
...
- [ ] Controller 完成跨问题汇总、全局覆盖与一轮受限补搜；证据不足或配额耗尽不自动写入正式计划；
...
- [ ] 默认图优先、两侧面板收起且可 toggle；刘看山轻量反馈不阻挡操作，静态降级可用；
- [ ] 用户可以直接编辑、拖动、完成和新增节点；
...
- [ ] 独立真实 M3/M4 与公网完整 Demo 按 §15.2 留有验收记录，Mock／缓存结果与真实调用区分；
- [ ] 产品说明计划书、作品链接和项目资料齐备，按 §15.3 在截止前发布并核对；
- [ ] OAuth 接入与否如实记录；若已接入，通过真实授权、回调安全、退出与会话隔离验收；
- [ ] 未接入官方黑客松要求的部分被显式标记，而不是假装完成。

---

## 附录 A：访谈维度参考（不作为运行时固定题库）

以下仅供访谈设计检查。实际问题及选项必须由模型根据目标和累计问答生成，不按此列表依次提问。
...
## 附录 C：2026-09-13 官方资料与核验边界

1. **赛事要求来源：** 用户于 2026-09-13 提供的六张官方指南截图，包含提报窗口、作品链接、计划书、OAuth 推荐、接口额度与刘看山素材入口。对应[官方参赛流程指南](https://pcnsiq9mmnww.feishu.cn/wiki/Pd1UwIIBriW0DBk8qlIczBAVnJc)；本次未能通过飞书连接重新读取全文，截图未覆盖的规则不视为已核验。
2. **技术资料来源：** 截图标注于 2026-09-12 更新的[官方指定 Skill 包 0.7.2-beta.20260911131715](https://developer-cdn.zhihu.com/zhihu-cli/releases/beta/skill/0.7.2-beta.20260911131715/zhihu-cli-skill-0.7.2-beta.20260911131715.zip)。已读取包内 `SKILL.md`、`references/hackathon.md`、`references/hackathon-oauth.md`、`references/hackathon-content-api.md`，并核对 `references/http-api.md` 的相关字段说明。包内部分赛事资料沿用 2026-09-03 记录，赛程与当前截图交叉核对；不能把包发布日期当成所有接口均已实测。
3. **待核验：** 当前账号实际额度与授权、赛事凭据领取状态、指定版本在部署环境的兼容性、公网 OAuth 和真实 M3/M4、刘看山素材包内容与使用说明。赛事截图的 5,000 次搜索额度与通用 Skill 默认额度不一致，按 §6.11 处理，不硬编码或承诺已获额度。
4. **范围决策：** 公网作品体验是 P0 发布门槛；用户计划只读分享仍为 P1。OAuth 推荐而非赛事普遍强制；Controller 预算、刘看山轻量交互及具体上线安全门槛是本产品设计决策，不冒充官方评分条款。
5. **操作边界：** 资料中的安装、升级、删除 Skill 或初始化账号说明仅作参考，不自动授权环境变更、读取个人数据、调用付费接口或提交作品。本次 PRD 修订未执行这些操作。

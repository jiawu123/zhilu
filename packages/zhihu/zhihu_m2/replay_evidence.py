"""Replay a saved live-demo source, optionally testing a narrower question.
...
--research-question changes ONLY the question passed to the existing compiler.
Original and effective inputs/hashes are recorded separately. A response to a
subquestion is NOT an answer to the complete original goal. Structural checks
are NOT a semantic quality evaluation. This tool does not aggregate evidence.
"""
import argparse
import hashlib
import json
...
REPLAY_VERSION = "m2-replay-v0.2.0"
...
def replay_saved_run(
    ...
    *,
    ...
    """Replay one checked source, changing only the question when requested.

    None preserves the original question. An override must be nonblank text of
    at most 2000 characters. No prompt mutation, extra model call or retry occurs.
    The manifest labels both questions so partial coverage cannot be silently
    presented as an answer to the original compound question.
    """
    if research_question is not None and (
        not isinstance(research_question, str)
        or not research_question.strip()
        or len(research_question) > 2000
    ):
        raise ValueError("research_question must be nonblank text of at most 2000 characters.")

    run_dir = Path(run_dir)
    result, original = _load_replay(run_dir)
    # Only replace this top-level scalar; the original record is not changed.
    frozen = dict(original)
    if research_question is not None:
        frozen["research_question"] = research_question
    question_changed = frozen["research_question"] != original["research_question"]
    source = frozen["source"]
    manifest = {
        "status": "dry_run", "compiler_version": ec.COMPILER_VERSION,
        "replay_version": REPLAY_VERSION,
        "run_kind": "question_scope_probe" if question_changed else "exact_replay",
        "question_changed": question_changed,
        "original_research_question": original["research_question"],
        "interpretation_scope": "single_source_effective_question",
        "original_frozen_input_sha256": _hash(json.dumps(original, ensure_ascii=False, sort_keys=True)),
    ...
    _write_json(folder / "original_replay_input.json", original)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Override only the research question; original inputs remain recorded."
    )
    args = parser.parse_args()
    try:
        manifest = replay_saved_run(
            args.run_dir, call_model=args.call_model, output_root=args.output_root,

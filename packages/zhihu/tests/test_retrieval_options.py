...
def test_default_is_batch_and_explicit_legacy_v3():
    assert options_from_env({}) == RetrievalOptions('batch-v1')
    assert options_from_env({'ZHIHU_RETRIEVAL_PROFILE': 'legacy'}) == RetrievalOptions('legacy')
    assert options_from_env({'ZHIHU_RETRIEVAL_PROFILE': 'v3'}) == RetrievalOptions('v3')
    assert options_from_env({'UNRELATED': 'private'}) == RetrievalOptions('batch-v1')
